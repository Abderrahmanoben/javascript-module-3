...
describe("04-exercises", () => {
  test("createUserObject returns a user object with the right properties", () => {
    ...
    const results = createUserObject("Alex", "Spence", "alex@mail.com");
    expect(results.firstName).toBeTruthy();
    expect(results.lastName).toBeTruthy();
    expect(results.email).toBeTruthy();
  });
  ...
  test("createCorporateEmailAddress appends the corporate email domain", () => {
    ...
    // Write the assertion
    expect(createCorporateEmailAddress("dani")).toMatch("dani@company.com");
  });
